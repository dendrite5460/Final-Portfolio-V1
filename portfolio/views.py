...
def dash(request):
	...
	PnL1=investments.objects.aggregate(t=Sum('gain_loss'))['t']
	PnL2=investmentsincommodities.objects.aggregate(t=Sum('gain_loss'))['t']
	PnL3=investmentsinfunds.objects.aggregate(t=Sum('gain_loss'))['t']
	PnL4=investmentsinbonds.objects.aggregate(t=Sum('gain_loss'))['t']
	PnL5=investmentsinfutures.objects.aggregate(t=Sum('gain_loss'))['t']
	PnL6=investmentsinindices.objects.aggregate(t=Sum('gain_loss'))['t']
	PnL=[PnL1, PnL2, PnL3, PnL4, PnL5, PnL6]
	...
	wl1= investments.objects.all().order_by('-id')[:2]
	wl2= investmentsinfutures.objects.all().order_by('-id')[:2]
	wl3= investmentsinbonds.objects.all().order_by('-id')[:1]
	ue= investmentsinfutures.objects.all().order_by('-id')[:5]
	last_stock=0
	v1=0
	assname=[]
	if request.method == 'POST' :
		if('assname1') in request.POST:
			...
			futures=['BAJAJ-AUTO','ASIANPAINT','EICHERMOT']
			p={'BAJAJ-AUTO':2618.15,'ASIANPAINT':1764.40  ,'EICHERMOT': 14762.55}
			price1=p[n1]
			c = CurrencyRates()
			...
			if(price1!=0.0):
				if(amount>price1):
					...
					import random
					from datetime import datetime
					start = datetime.strptime('5/2/2020 1:30 PM', '%m/%d/%Y %I:%M %p')
					end = datetime.strptime('12/1/2020 4:50 AM', '%m/%d/%Y %I:%M %p')
					delta = end - start
	...
	return render(request, 'portfolio/dash.html',{'price1':price1,'price2':price2,'price3':price3,'price4':price4,'price5':price5,'price6':price6,'n1':n1,'n3':n3,'n2':n2,'n4':n4,'n5':n5,'n6':n6,'v123':v123,'ass1':ass1,'no11':no11,'no22':no22,'close11':close11,'close22':close22,'labelsinall':labelsinall,'datapinall':datapinall,'PnL': PnL, 'n':n,'labels':labels,'datap':datap,'labels1':labels1,'datap1':datap1,'labels2':labels2,'datap2':datap2,'labels3':labels3,'datap3':datap3,'labels4':labels4,'datap4':datap4,'labels5':labels5,'datap5':datap5,'price':price,'amount':amount,'query_results1':query_results1,'query_results2':query_results2,'query_results3':query_results3,'query_results4':query_results4,'query_results5':query_results5,'query_results6':query_results6,'rem':rem,'close':close,'no':no,'s':s,'close1':close1,'close2':close2,'no1':no1,'no2':no2,'DataFrame3':DataFrame3, 'wl1': wl1, 'wl2': wl2, 'wl3': wl3, 'ue': ue})
def sales(request):
